// C++ port of main.c for ATmega328 (irrigation system)
// Converted to C++ style while keeping AVR APIs and behavior.
...
static const char *TRIGGERS_BUFFER[] = {
	"MIN CAPACITY",
	"MAX TEMP"
};
...
static char *MESSAGES_BUFFER[MSG_BUFFER_SIZE] = {(char*)"No message", (char*)"No message", (char*)"No message", (char*)"No message"};

static int active_menu_index = -1;
static int active_trigger_index = -1;
static int active_config_index = -1;
static int active_message_index = -1;

static int menu_hover_index = 0;
static int trigger_hover_index = 0;
static int config_hover_index = 0;
static int message_hover_index = 0;


// user application variables
static float MINIMUM_CAPACITY_BEFORE_REFILLING_TRIGGER = 1.0f;
static float MAXIMUM_TEMPERATURE_BEFORE_PUMPING = 28.5f;
static float STEP_SIZE_FOR_INCREMENTS = 0.5f;
static float STEP_INCREMENT = 0.25f;
static float PUMP_THRESHOLD = 20.0f;
static float SPRAY_THRESHOLD = 24.0f;
static char ENABLE_TRIGGER_VALUE = 1;
static char ENABLE_ALERT_VALUE = 1;
...
/*
	...
	returns: void
*/
static void LCD_1602A_send_nibble(uint8_t nibble)
{
...
/*
	...
	returns: void
*/
static void LCD_1602A_load_command(uint8_t cmd)
{
...
/*
	...
	returns: void
*/
static void LCD_1602A_load_data(uint8_t data)
{
...
/*
	...
	returns: void
*/
static void LCD_1602A_print(const char* str)
{
	while (*str)
	{
...
static void LCD_1602A_create_char(uint8_t location, uint8_t charmap[])
{
...
/* 
	...
	returns: uint16_t - the distance in centimeters
*/
static uint16_t HCSR04_get_distance()
{
	HCSR04_trigger();
	return HCSR04_read();
}
...
/* 
	...
	returns: void
*/
static void display_set(const unsigned char *title, const unsigned char *data)
{
	// LCD_1602A_load_command(DISPLAY_CLEAR_SCREEN);
	_delay_ms(2);
...
*/
static float get_tank_capacity()
{
	...
	float volume = (PI * ((float)TANK_RADIUS_IN_CM * (float)TANK_RADIUS_IN_CM) * (float)water_height) / 1000.0f; // convert cm^3 to liters
	return volume;
}
...
static float get_refill_rate()
{
	...
	if (capacity_at_end < capacity_at_start) {
		return 0.0f; // no refill detected
	}

	float refill_rate_per_second = (capacity_at_end - capacity_at_start) / 2.0f;

	return refill_rate_per_second * 60.0f; // convert to liters per minute
}
...
static float get_leak_rate()
{
	...
	if (capacity_at_start < capacity_at_end) {
		return 0.0f; // no leak detected
	}
	float leak_rate_per_second = (capacity_at_start - capacity_at_end) / 2.0f;

	return leak_rate_per_second * 60.0f; // convert to liters per minute
}

static float get_soil_temperature()
{
...
/* 
	...
	returns: void
*/
static void ui_show_display()
{
	if (active_menu_index == -1)
	{
		display_set((const unsigned char*)"MAIN MENU", (const unsigned char*)MENU_BUFFER[menu_hover_index]);
	}
	else
	{
		...
		{
		case 0:
			display_set((const unsigned char*)"TANK CAPACITY", (const unsigned char*)"Loading...");
			format_float(buffer, sizeof(buffer), get_tank_capacity(), 1, "Ltrs");
			display_set((const unsigned char*)"TANK CAPACITY", (const unsigned char*)buffer);
			break;
		case 1:
			display_set((const unsigned char*)"REFILL RATE", (const unsigned char*)"Loading...");
			format_float(buffer, sizeof(buffer), get_refill_rate(), 1, "Ltrs per min");
			display_set((const unsigned char*)"REFILL RATE", (const unsigned char*)buffer);
			break;
		case 2:
			display_set((const unsigned char*)"LEAKAGE RATE", (const unsigned char*)"Loading...");
			format_float(buffer, sizeof(buffer), get_leak_rate(), 1, "Ltrs per min");
			display_set((const unsigned char*)"LEAKAGE RATE", (const unsigned char*)buffer);
			break;
		case 3:
			display_set((const unsigned char*)"SOIL TEMPERATURE", (const unsigned char*)"Loading...");
			format_float(buffer, sizeof(buffer), get_soil_temperature(), 1, "degrees");
			display_set((const unsigned char*)"SOIL TEMPERATURE", (const unsigned char*)buffer);
			break;
		case 4:
			if (active_trigger_index == -1)
			{
				display_set((const unsigned char*)"TRIGGERS", (const unsigned char*)TRIGGERS_BUFFER[trigger_hover_index]);
			}
			else
			{
				...
				case 0:
					...
					display_set((const unsigned char*)"MIN CAPACITY", (const unsigned char*)buffer);
					break;
				case 1:
					format_float(buffer, sizeof(buffer), MAXIMUM_TEMPERATURE_BEFORE_PUMPING, 2, "degrees");
					display_set((const unsigned char*)"MAX TEMP", (const unsigned char*)buffer);
					break;
				default:
					display_set((const unsigned char*)"ERROR", (const unsigned char*)"Invalid trigger");
					break;
				}
			}
			break;
		...
		case 5:
			display_set((const unsigned char*)"MESSAGES", (const unsigned char*)MESSAGES_BUFFER[message_hover_index]);
			break;
		case 6:
			if (active_config_index == -1)
			{
				display_set((const unsigned char*)"CONFIG", (const unsigned char*)CONFIG_BUFFER[config_hover_index]);
			}
			else
			{
				...
				case 0:
					...
					display_set((const unsigned char*)"STEP SIZE", (const unsigned char*)buffer);
					break;
				case 1:
					format_float(buffer, sizeof(buffer), PUMP_THRESHOLD, 1, "Ltrs");
					display_set((const unsigned char*)"PUMP THRESHOLD", (const unsigned char*)buffer);
					break;
				case 2:
					format_float(buffer, sizeof(buffer), SPRAY_THRESHOLD, 1, "degrees");
					display_set((const unsigned char*)"SPRAY THRESHOLD", (const unsigned char*)buffer);
					break;
				case 3:
					snprintf(buffer, sizeof(buffer), "%s", ENABLE_TRIGGER_VALUE ? "Enabled" : "Disabled");
					display_set((const unsigned char*)"ENABLE TRIGGERS", (const unsigned char*)buffer);
					break;
				case 4:
					snprintf(buffer, sizeof(buffer), "%s", ENABLE_ALERT_VALUE ? "Enabled" : "Disabled");
					display_set((const unsigned char*)"ENABLE ALERTS", (const unsigned char*)buffer);
					break;
				default:
					display_set((const unsigned char*)"ERROR", (const unsigned char*)"Invalid config");
					break;
				}
			}
			break;
		...
		default:
			display_set((const unsigned char*)"ERROR", (const unsigned char*)"Invalid menu");
			break;
		}
	}
}
...
/* 
	...
	returns: void
*/
static void ui_process_key_command (uint8_t key) {
	if (key == 1)
	{
		if (active_menu_index == -1)
		{
